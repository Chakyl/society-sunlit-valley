  ...
  if (player.age % 200 == 0 && player.level.dimension === "society:skull_cavern") {
    ...
    if (timeModulo >= 17000) {
      if (timeModulo <= 17200) {
        ...
        server.runCommandSilent(
          `emberstextapi sendcustom ${player.username} {anchor:"BOTTOM_CENTER",background:1,align:"BOTTOM_CENTER",color:"#AAAAAA",offsetY:-60} 200 ${Text.translatable("society.skull_cavern.late").getString()}`
        );
      }
    ...
    if (timeModulo >= 20000) {
      if (timeModulo <= 20200) {
        server.runCommandSilent(
          `emberstextapi sendcustom ${player.username} {anchor:"BOTTOM_CENTER",charShakeRandom:0.2,background:1,align:"BOTTOM_CENTER",color:"#AAAAAA",offsetY:-60} 200 ${Text.translatable("society.skull_cavern.concern").getString()}`
        );
      }
      server.runCommandSilent(
    ...
    if (timeModulo >= 23800) {
      ...
      server.runCommandSilent(
        `emberstextapi sendcustom ${player.username} {anchor:"BOTTOM_CENTER",background:1,align:"BOTTOM_CENTER",color:"#AAAAAA",offsetY:-60} 200 ${Text.translatable("society.skull_cavern.fainted").getString()}`
      );
