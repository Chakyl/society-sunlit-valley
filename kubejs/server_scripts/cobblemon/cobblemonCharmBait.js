...
let spawnSkyLegendary = (server, level, player, birds, variant, sound, message) => {
    ...
    let spawnPos = player.getOnPos().offset(rnd(-64, 64), 94, rnd(-64, 64));
    let { x, y, z } = spawnPos;
...
PlayerEvents.tick((e) => {
    ...
    if (player.age % 18000 == 0 && player.isCuriosEquipped("sunlit_cobblemon:mystica_nectar") && Math.random() < 0.2) {
    ...
    if (player.age % 18000 == 0 && player.isCuriosEquipped('sunlit_cobblemon:rainbow_steam') && Math.random() < 0.2) {
