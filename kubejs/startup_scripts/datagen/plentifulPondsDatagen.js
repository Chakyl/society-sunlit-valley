...
Array.from(global.fishPondDefinitions.keys()).forEach((fishID) => {
    let pondData = global.fishPondDefinitions.get(fishID);
    let pondJson = {
        "color": "#ffffff",
        "fish": fishID,
        "max_population": 10,
    ...
    if (pondData.additionalRewards != undefined) {
        pondJson.drops = pondData.additionalRewards.map((reward) => ({
            "min_population": reward.minPopulation,
            "drop": {
                "id": reward.item,
                "count": reward.count
            },
    ...
    JsonIO.write(`kubejs/data/plentifulponds/ponds/${fishID.replace(":", "_")}.json`, pondJson);
});